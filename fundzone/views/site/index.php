...
$baseUrl = Url::base();
?>
<section class="banner homeBanner" style="margin-bottom: 0;"><a href="<?= Url::base().'/trade/purchase-page?fundcode=000277';?>" class="empLink" target="_blank">banner</a></section>
	...
	<div class="column hotList fixed">
		<ul id="recommend">
		    <?php foreach($recommend as $key=>$value): ?>
				...
				<li>
					<dl class="hotBlk">
						<dt class="blkT" style="margin-bottom: 30px;"><a class="colblue" href="<?= Yii::$app->params['homeHost'].'site/detail?fundcode='.$value['FundCode'];?>"><?= $value['FundAbbrName'];?> <span class="fs12 col96 dbk"><?= $value['FundCode'];?></span></a></dt>
						<dd class="fixed">
							<div class="fL">
						...
						<dd><a href="<?= Url::base().'/trade/purchase-page?fundcode='.$value['FundCode'];?>" class="buttonA">购买</a></dd>
					</dl>
				</li>
			<?php endforeach;?>
		</ul>
					...
					<?php if(isset($moneyFundList['list']) && !empty($moneyFundList['list'])):?>
						<?php foreach($moneyFundList['list'] as $key=>$value):?>
							<tr>
								...
								<td><a class="colblue" href="<?= Yii::$app->params['homeHost'].'site/detail?fundcode='.$value['FundCode'];?>"><span><?= $value['FundAbbrName'];?></span></a></td>
								...
								<td><span class="<?= $value['RRInSingleYear']>0?'colRed':($value['RRInSingleYear']<0?'colGreen':'');?>"><?= $value['RRInSingleYear']!==''?$value['RRInSingleYear'].'%':'--';?></span></td>
								<!-- <td><span>机货基中的战斗机</span></td> -->
								<td><a href="<?= Url::base().'/trade/purchase-page?fundcode='.$value['FundCode'];?>" class="buttonB">申购</a></td>
								<td>&nbsp;</td>
							</tr>
						<?php endforeach;?>
					<?php endif;?>
	...
	<div class="maxW newsBlkWarp fixed">
		...
		<?php foreach ($Categorys as $key => $value): ?> 
			<div class="newsBlk <?= $key%2?'odd':'even';?>">
				...
				<a href="<?= Yii::$app->params['homeHost'].'site/news?cid='.$value['id'];?>" class="more">更多</a>
				<ul>
					<?php foreach ($model->getRecommendNewsList($value['id']) as $k => $v):?>
						<?php if($k==0):?>
							<li class="topLine">
								<a href="<?= isset($v['Link']) && !empty($v['Link']) ? $v['Link'] : Yii::$app->params['homeHost'].'site/news?id='.$v['id'].'&cid='.$value['id'];?>">
