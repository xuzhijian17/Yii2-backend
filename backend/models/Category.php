...
class Category extends BaseModel
{
    ...
    {
        return [
            [['id','category'],'required','on'=>'editCat'],
            ['category','required','on'=>'addCat'],
            ['id','required','on'=>'delCat'],
        ];
    }
    ...
    public function catList($value='')
    {
        ...
        $sql ="SELECT *,fc.id,fc.UpdateTime,COUNT(fl.CategoryId) AS FundNums FROM {$tableName} fc LEFT JOIN fund_list_{$instid} fl ON fc.id=fl.CategoryId AND fl.`Status`!=-1 WHERE fc.Instid={$instid} AND fc.`Status`!=-1 GROUP BY fc.id ORDER BY fc.UpdateTime DESC";
    ...
    public function DelCat()
    {
        ...
        try {
            ...
            $sql = "UPDATE {$tableName} fc LEFT JOIN fund_list_{$instid} fl ON fc.id=fl.CategoryId SET fl.`Status`=-1 WHERE fc.id=:id AND fc.Instid=:instid";
            $command = self::getDb()->createCommand($sql);
            $command->bindParam(":id",$this->id);
            $command->bindParam(":instid",$instid);
            $rs2 = $command->execute();
